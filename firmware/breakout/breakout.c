...
static bool border_collision_handling(ball_t* ball);
...
void breakout(void)
{
    ...
    while (true) {
        ...
        is_game_over = border_collision_handling(&ball);
        ...
        // it is possible that the wall collision handling caused a border collision
        is_game_over = border_collision_handling(&ball);
...
{
    if (ball->x == -1) {
        // collision on left side of Wall detected!
        ball->x  = 1;
        ball->dx = 1;
    }

    if (ball->x == 8) {
        // collision on right side of Wall detected!
        ball->x  = 6;
        ball->dx = -1;
    }

    if (ball->y == -1) {
        // collision on uper Wall detected!
        ball->y  = 1;
        ball->dy = +1;
    }

    if (ball->y == 8) {
        // collision game Over detected!
        return true;
    }
